...
export default function MediaUploader({
  ...
  // Get file category
  const getFileCategory = useCallback((mimeType: string): 'images' | 'videos' | 'audio' | 'documents' => {
  ...
  const retryUpload = useCallback(async (taskId: string) => {
    ...
    try {
      setUploadTasks(prev => prev.map(t => 
        t.id === taskId ? { ...t, status: 'uploading', error: undefined } : t
      ));
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {uploadedFiles.map((file) => {
                ...
                return (
                  <div key={file.id} className="group relative bg-gray-50 rounded-lg p-4">
