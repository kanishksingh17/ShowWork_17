...
const generateMockSocialData = (): {
  ...
  const platforms = ['instagram', 'twitter', 'linkedin', 'youtube', 'facebook'] as const;
  ...
  platforms.forEach((platform, platformIndex) => {
    for (let i = 0; i < 15; i++) {
      ...
      posts.push({
        ...
        mediaUrl: i % 3 === 0 ? `https://example.com/media/${i}.jpg` : undefined,
        ...
        metrics: {
          ...
          saves: platform === 'instagram' ? Math.floor(engagement * 0.2) : undefined,
          views: platform === 'youtube' ? Math.floor(baseReach * 1.5) : undefined,
        },
  ...
  const platformMetrics: PlatformMetrics[] = platforms.map(platform => {
    ...
    const totalEngagement = platformPosts.reduce((sum, p) => sum + p.metrics.engagement, 0);
  ...
  const insights: ContentInsights = {
    ...
    crossPlatformReach: platformMetrics.reduce((sum, p) => sum + p.totalReach, 0),
    totalEngagement: platformMetrics.reduce((sum, p) => sum + p.totalEngagement, 0),
    viralContent: posts
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {((data.insights.totalEngagement / data.insights.crossPlatformReach) * 100).toFixed(1)}%
                </p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{data.posts.length}</p>
              ...
              {data.platformMetrics.map((platform, index) => (
                <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div className="flex items-center space-x-3">
                    <div className={`p-2 rounded-full text-white ${getPlatformColor(platform.platform)}`}>
            ...
            <div className="space-y-3">
              {data.insights.topHashtags.map((hashtag, index) => (
                <div key={index} className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-gray-600">{hashtag.reach.toLocaleString()} reach</span>
                </div>
              ))}
            </div>
          </CardContent>
              ...
              <div key={post.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-sm">{post.content.substring(0, 50)}...</p>
                ...
                <div className="text-right">
                  <p className="font-semibold text-sm">{post.engagementRate.toFixed(1)}%</p>
