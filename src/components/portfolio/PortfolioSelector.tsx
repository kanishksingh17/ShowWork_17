...
import { PortfolioTemplate, JobRole, AIRecommendation } from '../../types/portfolio';
...
export const PortfolioSelector: React.FC<PortfolioSelectorProps> = ({
  ...
  const industries = [
    ...
    'Education', 'E-commerce', 'Gaming', 'Media', 'Consulting'
  ];
  ...
  const detectJobRole = async () => {
    ...
    try {
      ...
      const recommendations = await aiService.getPortfolioRecommendations(jobRole, userData, projects);
  ...
  const filteredTemplates = templates.filter(template => {
    ...
    const matchesLayout = selectedLayout === 'all' || template.layout === selectedLayout;
    
    return matchesSearch && matchesIndustry && matchesLayout;
  });

  const getRecommendationForTemplate = (templateId: string): AIRecommendation | undefined => {
        ...
        <Card className="border-green-200 bg-green-50">
          ...
          <CardContent>
            <div className="space-y-2">
              <h3 className="font-semibold text-green-900">{detectedJobRole.title}</h3>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {aiRecommendations.slice(0, 3).map((recommendation) => {
              ...
              return (
                <Card key={template.id} className="border-yellow-200 bg-yellow-50 hover:shadow-lg transition-shadow">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-yellow-900">{template.name}</CardTitle>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      <div>
                        <h4 className="font-medium text-yellow-900 mb-1">Why this template?</h4>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filteredTemplates.map((template) => {
            ...
            const LayoutIcon = layouts.find(l => l.value === template.layout)?.icon || Code;
                  ...
                  <div className="space-y-3">
                    ...
                    {recommendation && (
                      <div className="p-3 bg-yellow-50 rounded-lg">
                        <h4 className="font-medium text-yellow-900 mb-1">AI Insights:</h4>
