...
export const PortfolioSystem: React.FC<PortfolioSystemProps> = ({
  ...
  const checkPrivacyStatus = async () => {
    try {
      const accepted = await securityManager.secureStorageGet('privacyAccepted');
  ...
  const loadAiInsights = async () => {
    ...
    setIsLoading(true);
    try {
      const aiService = new PortfolioAIService(process.env.REACT_APP_OPENAI_API_KEY || '');
      const jobRole = await aiService.extractJobRole(userData, projects);
      
      // Calculate success probability
      const successProbability = calculateSuccessProbability(jobRole, userData, projects);
      
      setAiInsights({
        jobRole,
        successProbability,
        recommendations: await aiService.getPortfolioRecommendations(jobRole, userData, projects)
      });
    } catch (error) {
  ...
  const calculateSuccessProbability = (jobRole: any, userData: any, projects: any[]) => {
    ...
    const experienceMatch = calculateExperienceMatch(userData.experience, jobRole.experienceLevel);
    score += experienceMatch * 0.2;
    
    // Industry alignment (10% weight)
    const industryAlignment = calculateIndustryAlignment(userData, jobRole.industry);
    score += industryAlignment * 0.1;
    
    return Math.min(Math.max(score, 0), 1);
  };

  const calculateSkillsMatch = (requiredSkills: string[], userSkills: string[]) => {
  ...
  const calculateExperienceMatch = (userExperience: string, requiredLevel: string) => {
    const experienceMap = {
      'entry': 0,
      'mid': 0.5,
      'senior': 0.8,
    ...
    const userLevel = userExperience.includes('5+') ? 0.8 : 
                     userExperience.includes('3+') ? 0.6 :
                     userExperience.includes('1+') ? 0.4 : 0.2;
    
    const requiredLevelValue = experienceMap[requiredLevel as keyof typeof experienceMap] || 0.5;
    
    return Math.min(userLevel / requiredLevelValue, 1);
  };

  const calculateIndustryAlignment = (userData: any, targetIndustry: string) => {
    // Simple industry alignment calculation
    const userIndustry = userData.industry || 'Technology';
    return userIndustry.toLowerCase() === targetIndustry.toLowerCase() ? 1 : 0.5;
  };
              ...
              <div className="space-y-6">
                  ...
                  <CardContent>
                    <div className="text-center space-y-4">
                      ...
                      <p className="text-green-700">
                        ...
                        <strong>{Math.round(aiInsights.successProbability * 100)}%</strong> chance 
                        of landing your target role: <strong>{aiInsights.jobRole.title}</strong>
                      </p>
                      <div className="w-full bg-green-200 rounded-full h-2">
                        <div 
                          className="bg-green-600 h-2 rounded-full transition-all duration-1000"
                          style={{ width: `${aiInsights.successProbability * 100}%` }}
                        ></div>
                      </div>
                    </div>
                  </CardContent>
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div>
                        ...
                        <div className="space-y-2 text-sm">
                          <p><strong>Title:</strong> {aiInsights.jobRole.title}</p>
                          <p><strong>Industry:</strong> {aiInsights.jobRole.industry}</p>
                          <p><strong>Level:</strong> {aiInsights.jobRole.experienceLevel}</p>
                          <p><strong>Description:</strong> {aiInsights.jobRole.description}</p>
                        </div>
                      </div>
                      <div>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Personalized suggestions to maximize your job application success
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                      ...
                      {aiInsights.recommendations.slice(0, 3).map((rec: any, index: number) => (
                        <div key={index} className="p-4 border rounded-lg">
                          <div className="flex items-center justify-between mb-2">
                            <h4 className="font-semibold">Template: {rec.templateId}</h4>
                            <Badge className="bg-blue-100 text-blue-800">
                              {Math.round(rec.confidence * 100)}% Match
                            </Badge>
                          </div>
                          <div className="space-y-2">
                            <h5 className="font-medium text-sm">Why this template?</h5>
              ...
              <Card>
                <CardContent className="text-center py-12">
                  ...
                  <p className="text-muted-foreground">
                    Please ensure you have provided your profile data and projects.
                  </p>
                </CardContent>
              </Card>
            )}
          ...
          <TabsContent value="analytics" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Track your portfolio's effectiveness and optimization opportunities
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid gap-4 md:grid-cols-3">
                  ...
                  <div className="text-center p-4 border rounded-lg">
                    ...
                    <div className="text-sm text-muted-foreground">Job Match Score</div>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-green-50 rounded-lg">
                    <h4 className="font-semibold text-green-900 mb-2">Skills Enhancement</h4>
                    <p className="text-sm text-green-700">
                      Consider adding certifications or projects in React and TypeScript 
                      to better match your target role requirements.
                    </p>
                  </div>
                  <div className="p-4 bg-yellow-50 rounded-lg">
                    <h4 className="font-semibold text-yellow-900 mb-2">Social Integration</h4>
                    <p className="text-sm text-yellow-700">
                      Update your LinkedIn profile to match your portfolio content 
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between p-4 border rounded-lg">
                  ...
                  <Badge className="bg-yellow-100 text-yellow-800">Opt-in</Badge>
                </div>
              </CardContent>
            </Card>
