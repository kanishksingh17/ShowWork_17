...
import { testAPIConnections, getAPIStatus, getHybridQuestions } from '../utils/webQuestionService';
...
const APITestComponent: React.FC = () => {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto bg-white rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold mb-6 text-gray-900">🌐 Improved Hybrid API System</h2>
      
      <div className="mb-4 p-3 bg-blue-50 rounded-lg">
        <p className="text-sm text-blue-800">
          🔄 <strong>Fixed OpenTDB Issues:</strong> Added JService API as primary source + multiple fallbacks
        </p>
      </div>
      
      {/* API Status */}
      <div className="mb-6 p-4 bg-gray-50 rounded-lg">
        <h3 className="text-lg font-semibold mb-3">📊 API Configuration Status</h3>
        {apiStatus && (
          <div className="space-y-2 text-sm">
            <p><strong>Real APIs Enabled:</strong> {apiStatus.realAPIsEnabled ? '✅ Yes' : '❌ No'}</p>
            <p><strong>Available APIs:</strong> {apiStatus.availableAPIs.join(', ') || 'None'}</p>
            <p><strong>OpenTDB:</strong> {apiStatus.config.ENABLE_OPENTDB ? '✅' : '❌'}</p>
            <p><strong>QuizAPI:</strong> {apiStatus.config.ENABLE_QUIZ_API ? '✅' : '❌'}</p>
          </div>
        )}
      </div>
      ...
      {testResults && (
        <div className="mb-6 p-4 bg-blue-50 rounded-lg">
          <h3 className="text-lg font-semibold mb-3">🔗 API Connection Results</h3>
      ...
      {sampleQuestions.length > 0 && (
        <div className="p-4 bg-green-50 rounded-lg">
          <h3 className="text-lg font-semibold mb-3">📝 Sample Questions (JavaScript)</h3>
      ...
      <div className="mt-6 p-4 bg-yellow-50 rounded-lg">
        ...
        <ul className="text-sm space-y-1 list-disc list-inside">
          <li>Click "Test API Connections" to verify external APIs are working</li>
