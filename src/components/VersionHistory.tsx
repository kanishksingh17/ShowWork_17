...
const VersionHistory: React.FC<VersionHistoryProps> = ({
  ...
  const loadVersionHistory = async () => {
    ...
    try {
      ...
      const storedVersions = localStorage.getItem(`project-versions-${projectId}`);
      if (storedVersions) {
        const parsedVersions = JSON.parse(storedVersions);
        setVersions(parsedVersions.sort((a: VersionSnapshot, b: VersionSnapshot) => 
  ...
  const generateVersionSnapshot = (project: any, action: string, changes: any[] = []): VersionSnapshot => {
    return {
  ...
  const saveVersionSnapshot = (snapshot: VersionSnapshot) => {
    ...
    localStorage.setItem(`project-versions-${projectId}`, JSON.stringify(updatedVersions));
  };

  const handleRevert = (version: VersionSnapshot) => {
    if (window.confirm(`Are you sure you want to revert to ${version.version}? This will overwrite your current changes.`)) {
      onRevert(version);
    }
  };

  const handleRestore = (version: VersionSnapshot) => {
    if (window.confirm(`Are you sure you want to restore ${version.version} as the current version?`)) {
      onRestore(version);
    }
  };
        ...
        {version.changes.map((change, index) => (
          <div key={index} className="border rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              <span className="font-medium text-sm capitalize">{change.field}</span>
            ...
            <div className="space-y-1">
              <div className="text-xs text-red-600 bg-red-50 p-2 rounded">
                <span className="font-medium">Before:</span> {change.oldValue || 'Empty'}
              </div>
              <div className="text-xs text-green-600 bg-green-50 p-2 rounded">
                <span className="font-medium">After:</span> {change.newValue || 'Empty'}
              </div>
            </div>
          </div>
        ))}
    ...
    return (
      <Card className={className}>
        <CardContent className="p-6 text-center">
          ...
          <p className="text-sm text-gray-500 mt-2">Loading version history...</p>
        </CardContent>
      </Card>
    );
  }
        ...
        <CardContent>
          {versions.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-400">Changes will be tracked automatically</p>
            </div>
          ) : (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className="ml-2">{new Date(selectedVersion.timestamp).toLocaleString()}</span>
                </div>
                <div>
