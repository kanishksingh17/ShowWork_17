...
import { Github, Loader2, CheckCircle, AlertCircle, ExternalLink } from 'lucide-react';
  ...
  const scrapeRepository = async () => {
    ...
    try {
      ...
      const [repoData, readmeData, languagesData] = await Promise.all([
        ...
        // Languages
        fetch(`https://api.github.com/repos/${repoInfo.owner}/${repoInfo.repo}/languages`)
          .then(res => res.json())
          .catch(() => ({}))
      ]);
      ...
      setError('Failed to scrape repository. Please check the URL and try again.');
    } finally {
      setIsScraping(false);
    }
  };

  const getTopLanguages = (languages: Record<string, number>, limit: number = 5) => {
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              {scrapedData.description && (
                <p className="text-gray-600 text-sm mt-1">{scrapedData.description}</p>
              )}
            </div>
            ...
            {scrapedData.readme && (
              <div>
                <h5 className="font-medium text-sm text-gray-700 mb-2">README Preview</h5>
