import { aiContentGenerator, UserInput, ContentSection } from './ai-content-generator'
...
export class AIContentService {
  ...
  async generateContent(request: ContentGenerationRequest): Promise<ContentGenerationResponse> {
    ...
    try {
      ...
      const enhancedInput: UserInput = {
        ...
        projects: request.input.projects || user.projects.map(project => ({
          name: project.name,
        ...
        skills: request.input.skills.length > 0 ? request.input.skills : user.skills.map(skill => skill.name),
      }

      // Generate content
      const content = await aiContentGenerator.generatePortfolioContent(enhancedInput)
      ...
      await this.logContentGeneration(request.userId, enhancedInput, content, generationTime)
  ...
  async regenerateContent(
    ...
    try {
      ...
      const updatedInput: UserInput = {
        ...
        colorPreferences: updates.colorPreferences || currentConfig.colorPreferences,
        language: updates.language || currentConfig.language || 'en',
        tone: updates.tone || currentConfig.tone || 'professional',
        targetAudience: updates.targetAudience || currentConfig.targetAudience || 'employers',
        includeTestimonials: updates.includeTestimonials || currentConfig.includeTestimonials || false,
        includeBlog: updates.includeBlog || currentConfig.includeBlog || false,
        includeResume: updates.includeResume || currentConfig.includeResume || true,
      }
   ...
   */
  private async saveContentToPortfolio(portfolioId: string, content: ContentSection): Promise<void> {
    try {
