...
export const AtomRenderer: React.FC<AtomRendererProps> = ({ 
  ...
  const variantData = atom.variants.find(v => v.name === variant) || atom.variants[0]
...
export const MoleculeRenderer: React.FC<MoleculeRendererProps> = ({ 
  ...
  const renderAtoms = () => {
    return molecule.atoms.map(atomId => {
...
export const OrganismRenderer: React.FC<OrganismRendererProps> = ({ 
  ...
  const renderSections = () => {
    return organism.sections.map(section => {
      const sectionMolecules = section.molecules.map(moleculeId => {
      ...
      <>
        {structure.header && (
          <motion.header
            ...
            transition={{ duration: 0.3 }}
          >
            {renderOrganisms().find((_, index) => 
              template.organisms[index] === 'header'
            )}
          </motion.header>
        )}
        ...
        {structure.main && (
          ...
          >
            {renderOrganisms().filter((_, index) => 
              !['header', 'hero', 'footer'].includes(template.organisms[index])
            )}
          </motion.main>
        )}
        ...
        {structure.footer && (
          ...
          >
            {renderOrganisms().find((_, index) => 
              template.organisms[index] === 'footer'
            )}
          </motion.footer>
        )}
      </>
...
export const ResponsiveWrapper: React.FC<ResponsiveWrapperProps> = ({ 
  ...
  const [currentBreakpoint, setCurrentBreakpoint] = React.useState<string>('desktop')
...
interface ComponentRendererProps {
  component: AtomComponent | MoleculeComponent | OrganismComponent | TemplateComponent
  props: ComponentProps
  atoms?: Map<string, AtomComponent>
  molecules?: Map<string, MoleculeComponent>
  organisms?: Map<string, OrganismComponent>
