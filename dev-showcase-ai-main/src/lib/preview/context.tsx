...
const devicePresets: DevicePreset[] = [
  // Desktop
  { id: 'desktop-1920', name: 'Desktop 1920x1080', width: 1920, height: 1080, type: 'desktop', orientation: 'landscape', pixelRatio: 1, category: 'popular' },
  { id: 'desktop-1440', name: 'Desktop 1440x900', width: 1440, height: 900, type: 'desktop', orientation: 'landscape', pixelRatio: 1, category: 'popular' },
  { id: 'desktop-1200', name: 'Desktop 1200x800', width: 1200, height: 800, type: 'desktop', orientation: 'landscape', pixelRatio: 1, category: 'popular' },
  
  // Tablet
  { id: 'ipad-pro', name: 'iPad Pro', width: 1024, height: 1366, type: 'tablet', orientation: 'portrait', pixelRatio: 2, category: 'popular' },
  { id: 'ipad', name: 'iPad', width: 768, height: 1024, type: 'tablet', orientation: 'portrait', pixelRatio: 2, category: 'popular' },
  { id: 'surface-pro', name: 'Surface Pro', width: 912, height: 1368, type: 'tablet', orientation: 'portrait', pixelRatio: 2, category: 'popular' },
  
  // Mobile
  { id: 'iphone-14-pro', name: 'iPhone 14 Pro', width: 393, height: 852, type: 'mobile', orientation: 'portrait', pixelRatio: 3, category: 'popular' },
  { id: 'iphone-12', name: 'iPhone 12', width: 390, height: 844, type: 'mobile', orientation: 'portrait', pixelRatio: 3, category: 'popular' },
  { id: 'pixel-7', name: 'Pixel 7', width: 412, height: 915, type: 'mobile', orientation: 'portrait', pixelRatio: 2.75, category: 'popular' },
  { id: 'galaxy-s21', name: 'Galaxy S21', width: 384, height: 854, type: 'mobile', orientation: 'portrait', pixelRatio: 2.75, category: 'popular' }
]
...
type PreviewAction =
  ...
  | { type: 'SET_EXPORT_STATUS'; payload: Partial<PreviewState['exportStatus']> }
  | { type: 'RESET_STATE' }
...
export const PreviewProvider: React.FC<PreviewProviderProps> = ({
  ...
  const [errorBoundaryState, setErrorBoundaryState] = React.useState<ErrorBoundaryState>({
    hasError: false,
    error: null,
    errorInfo: null,
    retryCount: 0,
  ...
  const connect = useCallback(async () => {
    try {
      setLoadingState(prev => ({ ...prev, isLoading: true, loadingType: 'initial', message: 'Connecting...' }))
      
      wsRef.current = websocketManager.createConnection('preview', finalConfig.websocketUrl, {
      ...
      wsRef.current.setOnExport((data) => {
        if (data.success) {
          dispatch({ type: 'SET_EXPORT_STATUS', payload: { downloadUrl: data.downloadUrl, progress: 100 } })
          onExportComplete?.(data)
        } else {
          dispatch({ type: 'SET_EXPORT_STATUS', payload: { error: data.error } })
        }
      })
  ...
  const setDeviceViewport = useCallback((viewport: DeviceViewport) => {
    ...
    setLoadingState(prev => ({ ...prev, isLoading: true, loadingType: 'device-switch', message: 'Switching device...' }))
  ...
  const addCustomViewport = useCallback((viewport: Omit<DeviceViewport, 'type'>) => {
  ...
  // Export actions
  const exportPortfolio = useCallback(async (options: ExportOptions): Promise<ExportResult> => {
    dispatch({ type: 'SET_EXPORT_STATUS', payload: { isExporting: true, exportType: options.type, progress: 0 } })
    
    try {
      setLoadingState(prev => ({ ...prev, isLoading: true, loadingType: 'export', message: `Exporting ${options.type}...` }))
      ...
      const result: ExportResult = {
        ...
        dispatch({ type: 'SET_EXPORT_STATUS', payload: { isExporting: false, downloadUrl: result.downloadUrl || null } })
      setLoadingState(prev => ({ ...prev, isLoading: false }))
      ...
      dispatch({ type: 'SET_EXPORT_STATUS', payload: { isExporting: false, error: result.error || null } })
      setLoadingState(prev => ({ ...prev, isLoading: false }))
  ...
  const cancelExport = useCallback(() => {
    dispatch({ type: 'SET_EXPORT_STATUS', payload: { isExporting: false, progress: 0 } })
    setLoadingState(prev => ({ ...prev, isLoading: false }))
  }, [])
  ...
  const clearError = useCallback(() => {
    dispatch({ type: 'SET_ERROR', payload: null })
    setErrorBoundaryState(prev => ({ ...prev, hasError: false, error: null, errorInfo: null }))
  }, [])
  ...
  const clearLoading = useCallback(() => {
    setLoadingState(prev => ({ ...prev, isLoading: false, progress: 0, message: '' }))
  }, [])
  ...
  }, [finalConfig.performanceMonitoring, enablePerformanceMonitoring, disablePerformanceMonitoring])
