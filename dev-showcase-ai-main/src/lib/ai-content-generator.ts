...
export const UserInputSchema = z.object({
  ...
  experience: z.number().min(0).max(50, 'Experience must be between 0-50 years'),
  ...
  colorPreferences: z.array(z.string().regex(/^#[0-9A-F]{6}$/i, 'Invalid hex color')).optional(),
  language: z.enum(['en', 'es', 'fr', 'de', 'it', 'pt', 'zh', 'ja', 'ko']).default('en'),
  tone: z.enum(['professional', 'casual', 'friendly', 'authoritative', 'creative']).default('professional'),
  targetAudience: z.enum(['employers', 'clients', 'peers', 'general']).default('employers'),
...
const LANGUAGE_CONFIGS = {
  en: {
    ...
    prompts: {
      ...
      about: 'Write an engaging about section highlighting {experience} years of experience',
      skills: 'Organize skills into relevant categories for a {role}',
      projects: 'Create detailed project descriptions showcasing technical expertise',
      ...
      testimonials: 'Generate authentic testimonials from colleagues and clients',
  ...
  es: {
    ...
    prompts: {
      ...
      about: 'Escribe una sección sobre mí atractiva destacando {experience} años de experiencia',
      skills: 'Organiza las habilidades en categorías relevantes para un {role}',
      projects: 'Crea descripciones detalladas de proyectos que muestren experiencia técnica',
  ...
  fr: {
    ...
    prompts: {
      ...
      about: 'Rédigez une section à propos engageante mettant en valeur {experience} années d\'expérience',
      skills: 'Organisez les compétences en catégories pertinentes pour un {role}',
      projects: 'Créez des descriptions détaillées de projets montrant l\'expertise technique',
      ...
      testimonials: 'Générez des témoignages authentiques de collègues et clients',
...
export class AIContentGenerator {
  ...
  async generatePortfolioContent(input: UserInput): Promise<ContentSection> {
    try {
      ...
      const [
        ...
        validatedInput.includeTestimonials ? this.generateTestimonialsSection(validatedInput) : null,
        validatedInput.includeBlog ? this.generateBlogSection(validatedInput) : null,
      ...
      // Cache the result
      this.setCachedContent(cacheKey, validatedContent, validatedInput.language)
   ...
   */
  private async generateHeroSection(input: UserInput): Promise<ContentSection['hero']> {
   ...
   */
  private async generateAboutSection(input: UserInput): Promise<ContentSection['about']> {
   ...
   */
  private async generateSkillsSection(input: UserInput): Promise<ContentSection['skills']> {
   ...
   */
  private async generateProjectsSection(input: UserInput): Promise<ContentSection['projects']> {
   ...
   */
  private async generateExperienceSection(input: UserInput): Promise<ContentSection['experience']> {
   ...
   */
  private async generateEducationSection(input: UserInput): Promise<ContentSection['education']> {
   ...
   */
  private async generateContactSection(input: UserInput): Promise<ContentSection['contact']> {
   ...
   */
  private async generateTestimonialsSection(input: UserInput): Promise<ContentSection['testimonials']> {
   ...
   */
  private async generateBlogSection(input: UserInput): Promise<ContentSection['blog']> {
   ...
   */
  private async generateSEOSection(input: UserInput): Promise<ContentSection['seo']> {
   ...
   */
  private buildPrompt(section: string, input: UserInput, context: Record<string, any>): string {
    const langConfig = LANGUAGE_CONFIGS[input.language] || LANGUAGE_CONFIGS.en
    const basePrompt = langConfig.prompts[section as keyof typeof langConfig.prompts]
...
`
      ...
      try {
        ...
        const response = await this.openai.chat.completions.create({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are an expert portfolio content writer. Generate professional, engaging content that matches the user\'s requirements. Always return valid JSON that matches the expected schema structure.'
            },
            {
              role: 'user',
              content: prompt
   ...
   * Set cached content
   */
  private setCachedContent(key: string, content: ContentSection, language: string): void {
    this.cache.set(key, {
      content,
      timestamp: Date.now(),
      language,
   ...
   */
  private async logGeneration(input: UserInput, content: ContentSection): Promise<void> {
    try {
   ...
   * Get cache stats
   */
  getCacheStats(): { size: number; entries: Array<{ key: string; timestamp: number; language: string }> } {
