...
let questionCache: CachedQuestions = {};
 ...
 */
async function fetchQuestionsFromWeb(techStack: string, count: number = 3): Promise<Question[]> {
  console.log(`🌐 Fetching ${count} questions for ${techStack} from web sources...`);
    ...
    for (let i = 0; i < sources.length; i++) {
      ...
      try {
        ...
        if (questions && questions.length > 0) {
          console.log(`✅ Successfully fetched ${questions.length} questions from ${i === 0 ? 'QuizAPI' : 'mock generator'}`);
          return questions.slice(0, count);
        }
      } catch (sourceError) {
        console.log(`⚠️ API source ${i + 1} failed, trying next...`, sourceError);
        continue;
      }
    }
...
async function fetchFromQuizAPI(techStack: string, count: number): Promise<Question[]> {
  ...
  await new Promise(resolve => setTimeout(resolve, API_CONFIG.QUIZ_API_DELAY));
  ...
  try {
    ...
    console.log(`🎯 Fetching from QuizAPI.io: ${url.replace(API_CONFIG.QUIZ_API_KEY, 'API_KEY')}`);
 ...
 */
async function fetchFromOpenTDB(techStack: string, count: number): Promise<Question[]> {
  ...
  try {
    ...
    if (data.response_code !== 0) {
      ...
      throw new Error(`OpenTDB API error ${data.response_code}: ${errorMessages[data.response_code] || 'Unknown error'}`);
    }
    ...
    const convertedQuestions = convertOpenTDBToQuestions(data.results, techStack);
 ...
 */
async function fetchFromJService(techStack: string, count: number): Promise<Question[]> {
  try {
 ...
 */
function convertQuizAPIToQuestions(apiData: any[], techStack: string): Question[] {
  return apiData.map((item, index) => ({
    ...
    correctAnswer: Object.keys(item.correct_answers).findIndex(key => item.correct_answers[key] === 'true'),
 ...
 */
function convertOpenTDBToQuestions(apiData: any[], techStack: string): Question[] {
  return apiData.map((item, index) => {
 ...
 */
function generateMockWebQuestions(techStack: string, count: number): Question[] {
 ...
 */
function generateBasicMockQuestions(techStack: string, count: number): Question[] {
  ...
  console.log(`🔧 Generated ${mockQuestions.length} basic mock questions for ${techStack}`);
  return mockQuestions;
}
 ...
 */
async function generateEnhancedWebQuestions(techStack: string, count: number): Promise<Question[]> {
  ...
  try {
    ...
    console.log(`⚠️ Mock questions failed, using basic fallback for ${techStack}`);
    return generateBasicMockQuestions(techStack, count);
  } catch (error) {
    console.error(`❌ Enhanced question generation failed for ${techStack}:`, error);
 ...
 */
function getCachedQuestions(techStack: string, count: number): Question[] | null {
  if (!isCacheValid(techStack)) return null;
  
  const cached = questionCache[techStack];
  console.log(`💾 Using cached questions for ${techStack} from ${cached.source}`);
 ...
 */
function cacheQuestions(techStack: string, questions: Question[], source: string): void {
  questionCache[techStack] = {
    questions,
    timestamp: Date.now(),
    source
  };
  console.log(`💾 Cached ${questions.length} questions for ${techStack} from ${source}`);
}
...
export async function getHybridQuestions(
  ...
  console.log(`🎯 Getting hybrid questions for ${techStack} (${count} questions)`);
 ...
 */
export async function testAPIConnections(): Promise<{ [source: string]: boolean }> {
  ...
  if (API_CONFIG.ENABLE_QUIZ_API && API_CONFIG.QUIZ_API_KEY !== 'YOUR_KEY') {
    try {
      const response = await fetch(`${API_SOURCES.quizAPI}?apiKey=${API_CONFIG.QUIZ_API_KEY}&limit=1`);
...
export function getAPIStatus(): { 
  ...
  if (API_CONFIG.ENABLE_JSERVICE) availableAPIs.push('JService (✅ Most Reliable)');
  if (API_CONFIG.ENABLE_OPENTDB) availableAPIs.push('OpenTDB (⚠️ May fail)');
  if (API_CONFIG.ENABLE_QUIZ_API && API_CONFIG.QUIZ_API_KEY !== 'demo-key') availableAPIs.push('QuizAPI (🔑 Requires key)');
