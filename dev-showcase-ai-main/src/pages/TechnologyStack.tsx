...
const TechnologyStack = ({ onBackToDashboard }: TechnologyStackProps) => {
  ...
  const popularTechnologies: Technology[] = [
    { id: '1', name: 'React', category: 'frontend', proficiency: 90, experience: '3+ years', projects: 15, isRecommended: true },
    { id: '2', name: 'TypeScript', category: 'frontend', proficiency: 85, experience: '2+ years', projects: 12, isRecommended: true },
    { id: '3', name: 'Node.js', category: 'backend', proficiency: 80, experience: '2+ years', projects: 10, isRecommended: true },
    { id: '4', name: 'Python', category: 'backend', proficiency: 75, experience: '1+ years', projects: 8, isRecommended: false },
    { id: '5', name: 'MongoDB', category: 'database', proficiency: 70, experience: '1+ years', projects: 6, isRecommended: false },
    { id: '6', name: 'PostgreSQL', category: 'database', proficiency: 65, experience: '1+ years', projects: 4, isRecommended: false },
    { id: '7', name: 'Docker', category: 'devops', proficiency: 60, experience: '1+ years', projects: 3, isRecommended: false },
    { id: '8', name: 'AWS', category: 'devops', proficiency: 55, experience: '6+ months', projects: 2, isRecommended: false },
    { id: '9', name: 'React Native', category: 'mobile', proficiency: 70, experience: '1+ years', projects: 5, isRecommended: false },
    { id: '10', name: 'TensorFlow', category: 'ai-ml', proficiency: 40, experience: '6+ months', projects: 1, isRecommended: false }
  ];
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">Manage your technology skills and expertise</p>
            </div>
        ...
        <div className="flex-1 overflow-auto p-6">
          <div className="space-y-6">
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-2">
                    {selectedTechnologies.map(tech => (
                      <Badge key={tech.id} variant="secondary" className="flex items-center gap-2 px-3 py-2">
                        <span className="font-medium">{tech.name}</span>
                        <span className={`text-xs px-2 py-1 rounded-full ${getProficiencyBg(tech.proficiency)} ${getProficiencyColor(tech.proficiency)}`}>
                          {tech.proficiency}%
                        </span>
                        <X 
            ...
            <Card className="bg-blue-50 border-blue-200">
              ...
              <CardContent>
                <p className="text-blue-800 mb-4">Based on your profile and current trends, here are some technologies you might want to explore:</p>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {technologies.filter(tech => tech.isRecommended).map(tech => (
                    <div key={tech.id} className="bg-white rounded-lg p-4 border border-blue-200 hover:shadow-md transition-shadow">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className={`font-medium ${getProficiencyColor(tech.proficiency)}`}>
                            {tech.proficiency}%
                          </span>
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredTechnologies.map(tech => (
                    <div key={tech.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200 hover:shadow-md transition-shadow">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-medium text-gray-900">{tech.name}</h4>
                        {tech.isRecommended && <Star className="w-4 h-4 text-yellow-500" />}
                      </div>
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-600">Proficiency</span>
                        ...
                        <Button 
                          ...
                          className="w-full mt-2"
                          onClick={() => addTechnology(tech)}
                          disabled={selectedTechnologies.some(t => t.id === tech.id)}
                        >
            ...
            <Card className="bg-white border border-gray-200">
              <CardHeader>
                <CardTitle className="text-lg font-semibold text-gray-900">Technology Categories</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {categories.slice(1).map(category => {
                    ...
                    const selectedCategoryTechs = selectedTechnologies.filter(tech => tech.category === category.id);
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium text-[#1E3A8A]">
                              {selectedCategoryTechs.length}/{categoryTechs.length}
                            </span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div 
                              className="bg-[#1E3A8A] h-2 rounded-full"
                              style={{ width: `${(selectedCategoryTechs.length / categoryTechs.length) * 100}%` }}
                            ></div>
                          </div>
                            ...
                            {selectedCategoryTechs.length > 0 ? (
                              <div className="flex flex-wrap gap-1">
                                {selectedCategoryTechs.map(tech => (
                                  <span key={tech.id} className="bg-[#1E3A8A] text-white px-2 py-1 rounded text-xs">
                                    {tech.name}
                                  </span>
                                ))}
                              </div>
