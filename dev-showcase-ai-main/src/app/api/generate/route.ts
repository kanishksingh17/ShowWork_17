...
import { withAuth, createErrorResponse, createSuccessResponse, trackUsage, checkUsageLimit, hasFeatureAccess } from '../../../lib/api/middleware'
import { GenerateContentRequestSchema, GenerateContentResponse } from '../../../lib/api/schemas'
  ...
  const response = await fetch(OPENAI_API_URL, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a professional content writer specializing in portfolio and personal branding content. Create engaging, professional, and conversion-focused content.'
        },
        {
          role: 'user',
          content: prompt
...
function generateSuggestions(content: string): string[] {
  ...
  if (!content.match(/\d+/)) {
    suggestions.push('Include specific numbers or statistics to add credibility')
  }
