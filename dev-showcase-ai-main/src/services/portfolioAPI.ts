...
import { TemplateMetadata, TEMPLATE_DATABASE, selectOptimalTemplate } from './templateEngine';
import { EnhancedContent, processUserDataWithAI, RawUserData } from './aiProcessor';
import { PortfolioCanvas } from '../components/3D/PortfolioCanvas';
import { DeploymentConfig, deployPortfolio, DeploymentResult } from './deploymentService';
...
export interface GenerationStatus {
  jobId: string;
  status: 'queued' | 'processing' | 'generating' | 'deploying' | 'completed' | 'failed';
  progress: number;
  currentStep: string;
  result?: {
    portfolioUrl: string;
...
export class PortfolioGenerationAPI {
      ...
      deployPortfolio: async (config: DeploymentConfig) => {
        ...
        return {
          ...
          analytics: {
            ...
            customEvents: ['portfolio_view', 'project_click', 'contact_form_submit']
          }
        };
      }
    };
  ...
  async generatePortfolio(request: GenerationRequest): Promise<GenerationResponse> {
  ...
  private async processGeneration(jobId: string, request: GenerationRequest) {
    try {
      ...
      const template = await this.selectTemplate(request.templateId, enhancedContent);
      ...
      const deploymentResult = await this.deploymentService.deployPortfolio(deploymentConfig);
  ...
  private async collectUserData(request: GenerationRequest): Promise<RawUserData> {
  ...
  private async selectTemplate(templateId?: string, content?: EnhancedContent): Promise<TemplateMetadata> {
    if (templateId) {
  ...
  private updateJobStatus(jobId: string, status: GenerationStatus) {
    ...
    console.log(`Job ${jobId}: ${status.status} - ${status.currentStep} (${status.progress}%)`);
  }
}
...
export const generatePortfolio = async (request: GenerationRequest): Promise<GenerationResponse> => {
