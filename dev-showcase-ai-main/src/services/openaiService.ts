...
export class PortfolioContentGenerator {
  ...
  async generateProfessionalBio(userData: any): Promise<string> {
    ...
    const response = await this.openai.createChatCompletion({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a professional copywriter specializing in creating compelling bios for tech professionals and developers.'
        },
        {
          role: 'user',
          content: prompt
  ...
  async generateProjectDescription(project: any): Promise<string> {
    ...
    const response = await this.openai.createChatCompletion({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a technical writer specializing in describing software projects for developer portfolios.'
        },
        {
          role: 'user',
          content: prompt
  ...
  async generateSEOMetadata(userData: any): Promise<{
    ...
    const response = await this.openai.createChatCompletion({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an SEO specialist creating optimized metadata for professional portfolio websites.'
        },
        {
          role: 'user',
          content: prompt
    ...
    try {
      ...
      return {
        ...
        keywords: userData.skills?.slice(0, 5) || ['developer', 'portfolio', 'web development']
      };
    }
  }
  ...
  async generateAchievementDescription(achievement: any): Promise<string> {
    ...
    const response = await this.openai.createChatCompletion({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a professional writer creating achievement descriptions for resumes and portfolios.'
        },
        {
          role: 'user',
          content: prompt
  ...
  async generateColorPalette(industry: string, preferences?: any): Promise<string[]> {
    ...
    const response = await this.openai.createChatCompletion({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a UI/UX designer specializing in color theory and professional web design.'
        },
        {
          role: 'user',
          content: prompt
  ...
  async generateCallToAction(context: string): Promise<{
    ...
    const response = await this.openai.createChatCompletion({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a conversion copywriter specializing in effective call-to-action text for professional websites.'
        },
        {
          role: 'user',
          content: prompt
...
export const generatePortfolioContent = async (userData: any, apiKey: string) => {
