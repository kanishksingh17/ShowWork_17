...
export const selectOptimalTemplate = (userProfile: UserProfile, templates: TemplateMetadata[]) => {
  return templates
    .map(template => ({
      template,
...
const calculateCompatibility = (profile: UserProfile, template: TemplateMetadata): number => {
  let score = 0;
  ...
  const avgSkillLevel = profile.skills.reduce((sum, skill) => sum + skill.level, 0) / profile.skills.length;
  if (avgSkillLevel > 80 && template.complexity === 'advanced') score += 25;
  else if (avgSkillLevel > 50 && template.complexity === 'intermediate') score += 25;
  else if (avgSkillLevel <= 50 && template.complexity === 'beginner') score += 25;
  
  // Project count matching (20% weight)
  if (profile.projects.length > 5 && template.features.includes('Project Carousel')) score += 20;
  else if (profile.projects.length <= 3 && template.features.includes('Project Grid')) score += 20;
...
const getCategoryFromProfile = (profile: UserProfile): TemplateMetadata['category'] => {
  const skills = profile.skills.map(s => s.name.toLowerCase());
  
  if (skills.some(s => ['react', 'node.js', 'python', 'javascript'].includes(s))) {
    return 'developer';
  }
  if (skills.some(s => ['design', 'figma', 'photoshop', 'illustrator'].includes(s))) {
    return 'creative';
  }
  if (profile.experience.some(exp => exp.position.toLowerCase().includes('manager') || exp.position.toLowerCase().includes('director'))) {
    return 'business';
  }
  if (profile.education.length > 0 && profile.education[0].degree.toLowerCase().includes('phd')) {
    return 'academic';
  }
...
export const TEMPLATE_DATABASE: TemplateMetadata[] = [
  // Developer Templates (100+)
  {
    ...
    complexity: 'advanced',
    performance: 95,
    features: ['3D Animations', 'Code Visualization', 'GitHub Integration', 'Dark Theme', 'Particle Effects'],
  ...
  },
  {
    ...
    performance: 92,
    features: ['Terminal UI', 'Command Line', 'Matrix Effects', 'Cyberpunk Theme'],
  ...
  // Creative Templates (100+)
  {
    ...
    complexity: 'intermediate',
    performance: 92,
    features: ['3D Gallery', 'Art Showcase', 'Interactive Elements', 'Light Theme'],
  ...
  // Business Templates (100+)
  {
    ...
    complexity: 'beginner',
    performance: 98,
    features: ['Data Charts', 'Professional Layout', 'Contact Forms', 'Analytics'],
  ...
  // Academic Templates (100+)
  {
    ...
    complexity: 'intermediate',
    performance: 94,
    features: ['Publication Gallery', 'Research Timeline', 'Citation Manager', 'Academic Theme'],
  ...
  // Freelancer Templates (100+)
  {
    ...
    complexity: 'advanced',
    performance: 91,
    features: ['Service Cards', 'Testimonials', 'Pricing Tables', 'Contact Integration'],
...
export const TEMPLATE_CATEGORIES = [
  ...
  { id: 'developer', name: 'Developer', count: TEMPLATE_DATABASE.filter(t => t.category === 'developer').length },
  { id: 'creative', name: 'Creative', count: TEMPLATE_DATABASE.filter(t => t.category === 'creative').length },
  { id: 'business', name: 'Business', count: TEMPLATE_DATABASE.filter(t => t.category === 'business').length },
  { id: 'academic', name: 'Academic', count: TEMPLATE_DATABASE.filter(t => t.category === 'academic').length },
  { id: 'freelancer', name: 'Freelancer', count: TEMPLATE_DATABASE.filter(t => t.category === 'freelancer').length }
];
...
export const getRecommendedTemplates = (userProfile: UserProfile): TemplateMetadata[] => {
...
export const optimizeTemplateForDevice = (template: TemplateMetadata, isMobile: boolean): TemplateMetadata => {
