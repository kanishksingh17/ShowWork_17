...
export class S3Client {
  ...
  async createBucket(): Promise<void> {
    ...
    const response = await fetch(url, {
      method: 'PUT',
      headers: {
        'Authorization': this.generateAuthHeader('PUT', `/${this.config.bucketName}`, ''),
  ...
  async putObject(object: S3Object): Promise<S3UploadResult> {
    ...
    const response = await fetch(url, {
      method: 'PUT',
      headers: {
        'Authorization': this.generateAuthHeader('PUT', `/${object.Key}`, object.Body),
  ...
  async putBucketWebsite(config: S3WebsiteConfig): Promise<void> {
    ...
    const response = await fetch(url, {
      method: 'PUT',
      headers: {
        'Authorization': this.generateAuthHeader('PUT', `/${this.config.bucketName}?website`, websiteConfig),
        'Content-Type': 'application/xml',
  ...
  async listObjects(prefix?: string): Promise<string[]> {
    ...
    const response = await fetch(url, {
      method: 'GET',
      headers: {
        'Authorization': this.generateAuthHeader('GET', `/?${prefix ? `prefix=${encodeURIComponent(prefix)}&` : ''}list-type=2`, ''),
      },
    });
  ...
  private generateAuthHeader(method: string, path: string, body: string | Buffer): string {
...
export class PortfolioAssetManager {
  ...
  async uploadPortfolioAssets(portfolioId: string, assets: {
    html: string;
    css: string;
    js: string;
  ...
  async uploadOptimizedAssets(portfolioId: string, optimizedAssets: {
    html: string;
    css: string;
    js: string;
  ...
  async deletePortfolioAssets(portfolioId: string): Promise<void> {
    ...
    await Promise.all(
      keys.map(key => this.s3.deleteObject(key))
    );
  }
  
  // Get asset URLs for CDN
  getAssetUrls(portfolioId: string, cdnDomain?: string): {
    html: string;
    css: string;
    js: string;
    images: string[];
  } {
    const baseUrl = cdnDomain || `https://${this.bucketName}.s3.${this.s3['config'].region}.amazonaws.com`;
    const prefix = `portfolios/${portfolioId}/`;
...
export const optimizeAndUploadAssets = async (
  ...
  // Upload optimized assets
  const result = await assetManager.uploadOptimizedAssets(portfolioId, optimizedAssets);
