...
import { Environment, OrbitControls, Text, Float, MeshDistortMaterial, useGLTF } from '@react-three/drei';
...
export const usePerformanceOptimization = () => {
  ...
  useEffect(() => {
    ...
    gl.setAnimationLoop((time) => {
      ...
      if (time % 1000 < 16) { // Check every second
        const info = gl.info;
...
export const useLOD = (distance: number) => {
  ...
  useFrame(() => {
    const distanceToCamera = camera.position.distanceTo(new THREE.Vector3(0, 0, 0));
...
export const ProfessionalHero3D: React.FC<ProfessionalHero3DProps> = ({
  title,
  subtitle,
  ctaText,
...
const ParticleSystem: React.FC<ParticleSystemProps> = ({ count, color = '#3B82F6', size = 0.02 }) => {
...
export const InteractiveProjectShowcase: React.FC<InteractiveProjectShowcaseProps> = ({
  projects,
  onProjectSelect
}) => {
  const groupRef = useRef<THREE.Group>(null);
...
const SkillsBars: React.FC<{ skills: Array<{ name: string; level: number }> }> = ({ skills }) => {
  return (
    <group>
...
const SkillsSpheres: React.FC<{ skills: Array<{ name: string; level: number }> }> = ({ skills }) => {
  return (
    <group>
      {skills.map((skill, index) => {
        ...
        return (
          <Float key={skill.name} speed={1} rotationIntensity={0.5} floatIntensity={1}>
            <mesh position={[x, 0, z]}>
...
const SkillsNetwork: React.FC<{ skills: Array<{ name: string; level: number; category: string }> }> = ({ skills }) => {
  return (
    <group>
      {skills.map((skill, index) => {
        const angle = (index / skills.length) * Math.PI * 2;
        ...
        return (
          <Float key={skill.name} speed={0.5} rotationIntensity={0.2} floatIntensity={0.5}>
            <mesh position={[x, 0, z]}>
...
export const Professional3DCanvas: React.FC<Professional3DCanvasProps> = ({
  children,
  performance = 'medium',
  background = '#0F172A',
