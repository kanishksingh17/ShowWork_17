...
export const PerformanceOverlay: React.FC<PerformanceOverlayProps> = ({ className = '' }) => {
  const { state, actions } = usePreview()
  const [isVisible, setIsVisible] = useState(false)
  const [isMinimized, setIsMinimized] = useState(false)
  const [selectedTab, setSelectedTab] = useState<'overview' | 'metrics' | 'charts' | 'settings'>('overview')
      ...
      >
        ...
        <div className="flex items-center justify-between p-3 border-b border-gray-200">
          <div className="flex items-center space-x-2">
            ...
            <StatusIcon className={`w-4 h-4 text-${performanceStatus.color}-600`} />
          </div>
        ...
        {!isMinimized && (
          <>
            ...
            <div className="p-3">
              {selectedTab === 'overview' && (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-1">
                      <StatusIcon className={`w-4 h-4 text-${performanceStatus.color}-600`} />
                      <span className={`text-sm font-medium capitalize text-${performanceStatus.color}-600`}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-gray-50 rounded p-2">
                      ...
                      <div className={`text-lg font-bold ${getFPSColor(state.performanceMetrics.fps)}`}>
                    ...
                    <div className="bg-gray-50 rounded p-2">
                      ...
                      <div className={`text-sm font-bold ${getMemoryColor(state.performanceMetrics.memoryUsage)}`}>
                    ...
                    <div className="bg-gray-50 rounded p-2">
                      ...
                      <div className={`text-sm font-bold ${getRenderTimeColor(state.performanceMetrics.renderTime)}`}>
                  ...
                  <div className="bg-gray-50 rounded p-2">
                    ...
                    <div className="text-sm text-gray-900">
                      {state.deviceViewport.name} ({state.deviceViewport.width}×{state.deviceViewport.height})
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-3">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className={`font-medium ${getFPSColor(state.performanceMetrics.fps)}`}>
                        {state.performanceMetrics.fps}
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${
                          state.performanceMetrics.fps >= 55 ? 'bg-green-500' :
                          state.performanceMetrics.fps >= 30 ? 'bg-yellow-500' : 'bg-red-500'
                        }`}
                        style={{ width: `${Math.min(100, (state.performanceMetrics.fps / 60) * 100)}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className={`font-medium ${getMemoryColor(state.performanceMetrics.memoryUsage)}`}>
                        {formatBytes(state.performanceMetrics.memoryUsage)}
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${
                          state.performanceMetrics.memoryUsage < 50 * 1024 * 1024 ? 'bg-green-500' :
                          state.performanceMetrics.memoryUsage < 100 * 1024 * 1024 ? 'bg-yellow-500' : 'bg-red-500'
                        }`}
                        style={{ width: `${Math.min(100, (state.performanceMetrics.memoryUsage / (200 * 1024 * 1024)) * 100)}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className={`font-medium ${getRenderTimeColor(state.performanceMetrics.renderTime)}`}>
                        {formatTime(state.performanceMetrics.renderTime)}
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${
                          state.performanceMetrics.renderTime < 16 ? 'bg-green-500' :
                          state.performanceMetrics.renderTime < 33 ? 'bg-yellow-500' : 'bg-red-500'
                        }`}
                        style={{ width: `${Math.min(100, (state.performanceMetrics.renderTime / 100) * 100)}%` }}
                      />
                    </div>
                  </div>
              ...
              {selectedTab === 'charts' && (
                <div className="space-y-3">
                  <div className="text-sm text-gray-600 mb-2">Performance History</div>
                  
                  {/* FPS Chart */}
                  <div className="space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-gray-600">FPS</span>
                      <span className={getFPSColor(state.performanceMetrics.fps)}>
                        {state.performanceMetrics.fps}
                      </span>
                    </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className={getMemoryColor(state.performanceMetrics.memoryUsage)}>
                        {formatBytes(state.performanceMetrics.memoryUsage)}
                      </span>
                    </div>
                    <div className="h-8 bg-gray-100 rounded flex items-end space-x-px p-1">
                      {history.slice(-20).map((metric, index) => (
                        <div
                          key={index}
                          className={`flex-1 ${
                            metric.memoryUsage < 50 * 1024 * 1024 ? 'bg-green-500' :
                            metric.memoryUsage < 100 * 1024 * 1024 ? 'bg-yellow-500' : 'bg-red-500'
                          }`}
                          style={{ height: `${Math.min(100, (metric.memoryUsage / (200 * 1024 * 1024)) * 100)}%` }}
                        />
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-gray-600">Render Time</span>
                      <span className={getRenderTimeColor(state.performanceMetrics.renderTime)}>
                        {formatTime(state.performanceMetrics.renderTime)}
                      </span>
                    </div>
                    <div className="h-8 bg-gray-100 rounded flex items-end space-x-px p-1">
                      {history.slice(-20).map((metric, index) => (
                        <div
                          key={index}
                          className={`flex-1 ${
                            metric.renderTime < 16 ? 'bg-green-500' :
                            metric.renderTime < 33 ? 'bg-yellow-500' : 'bg-red-500'
                          }`}
                          style={{ height: `${Math.min(100, (metric.renderTime / 100) * 100)}%` }}
                        />
                      ))}
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-5 w-9 items-center rounded-full ${
                          state.performanceMetrics ? 'bg-blue-600' : 'bg-gray-300'
                        }`}
                      >
                        <span
                          className={`inline-block h-3 w-3 transform rounded-full bg-white transition ${
                            state.performanceMetrics ? 'translate-x-5' : 'translate-x-1'
                          }`}
                        />
                      </button>
                    </div>
                  ...
                  <div className="pt-2 border-t border-gray-200">
                    <div className="text-xs text-gray-500">
                      Last Updated: {state.performanceMetrics.lastUpdated.toLocaleTimeString()}
                    </div>
                  </div>
                </div>
              )}
