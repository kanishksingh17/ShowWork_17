...
router.get("/profile", requireAuth, async (req, res) => {
  try {
    const user = await User.findById(req.user._id).select(
      "-password -googleId -githubId -stripeCustomerId -stripeSubscriptionId",
    );
    ...
    res.json({
      success: true,
      user,
      portfolioUrl: `http://localhost:3000/portfolio/${user.username}`,
    });
  } catch (error) {
...
router.get("/public/:username", async (req, res) => {
  try {
    ...
    const user = await User.findOne({ username }).select(
      "-password -googleId -githubId -stripeCustomerId -stripeSubscriptionId -email",
    );
...
router.put("/profile", requireAuth, async (req, res) => {
  try {
    const { tagline, bio, location, website, socials, portfolioSettings } =
      req.body;
    ...
    if (portfolioSettings !== undefined)
      updateData.portfolioSettings = portfolioSettings;
...
router.get("/skills", requireAuth, async (req, res) => {
  try {
    const skills = await Skill.find({ isActive: true }).select(
      "name displayName category description icon color totalQuestions timeLimit",
    );
...
router.get("/:username", async (req, res) => {
  try {
    ...
    const user = await User.findOne({ username }).select(
      "name tagline bio avatar location website socials skills achievements projects portfolioSettings",
    );
