...
Cypress.Commands.add('createPortfolio', (title: string, description: string) => {
...
Cypress.Commands.add('measurePerformance', () => {
  cy.window().then((win) => {
    ...
    const metrics = {
      ...
      firstInputDelay: performance.getEntriesByName('first-input')[0]?.processingStart - performance.getEntriesByName('first-input')[0]?.startTime || 0
    }
...
Cypress.Commands.add('mockAPI', (method: string, url: string, response: any) => {
  if (response.fixture) {
    cy.fixture(response.fixture).then((fixtureData) => {
      cy.intercept(method, url, fixtureData).as(`${method.toLowerCase()}_${url.replace(/\//g, '_')}`)
    })
  } else {
    cy.intercept(method, url, response).as(`${method.toLowerCase()}_${url.replace(/\//g, '_')}`)
  }
})
...
Cypress.Commands.add('clearMocks', () => {
  ...
  cy.intercept('DELETE', '/api/**', { fixture: 'empty.json' }).as('clearDelete')
})
...
Cypress.Commands.add('verifyAPICall', (method: string, url: string) => {
  cy.get(`@${method.toLowerCase()}_${url.replace(/\//g, '_')}`).should('have.been.called')
})
