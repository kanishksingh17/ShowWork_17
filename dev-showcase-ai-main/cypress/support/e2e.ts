...
beforeEach(() => {
  ...
  cy.intercept("POST", "/api/auth/signin", { fixture: "signin.json" }).as(
    "signIn",
  );
  cy.intercept("POST", "/api/auth/signout", { fixture: "signout.json" }).as(
    "signOut",
  );

  // Mock API responses
  cy.intercept("GET", "/api/portfolios*", { fixture: "portfolios.json" }).as(
    "getPortfolios",
  );
  cy.intercept("POST", "/api/portfolios", { fixture: "portfolio.json" }).as(
    "createPortfolio",
  );
  cy.intercept("GET", "/api/portfolios/1", { fixture: "portfolio.json" }).as(
    "getPortfolio",
  );
  cy.intercept("PUT", "/api/portfolios/1", { fixture: "portfolio.json" }).as(
    "updatePortfolio",
  );
  cy.intercept("DELETE", "/api/portfolios/1", { statusCode: 200 }).as(
    "deletePortfolio",
  );
  cy.intercept("POST", "/api/generate", {
    fixture: "generated-content.json",
  }).as("generateContent");
  cy.intercept("POST", "/api/preview", { fixture: "preview.json" }).as(
    "generatePreview",
  );
  ...
  cy.intercept("POST", "https://api.vercel.com/v13/deployments", {
    fixture: "vercel-deployment.json",
  }).as("vercel");
