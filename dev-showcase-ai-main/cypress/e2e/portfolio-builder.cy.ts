describe('Portfolio Builder E2E Tests', () => {
  ...
  describe('Portfolio Creation Flow', () => {
    it('should create a new portfolio', () => {
      ...
      cy.get('[data-testid="portfolio-description"]').type('A test portfolio for E2E testing')
      ...
      cy.get('[data-testid="portfolio-title-display"]').should('contain', 'My Test Portfolio')
    })
    ...
    it('should preview portfolio in different devices', () => {
      ...
      cy.get('[data-testid="preview-container"]').should('have.css', 'width', '1200px')
      
      // Test tablet preview
      cy.get('[data-testid="device-selector"]').click()
      cy.get('[data-testid="device-tablet"]').click()
      cy.get('[data-testid="preview-container"]').should('have.css', 'width', '768px')
      
      // Test mobile preview
      cy.get('[data-testid="device-selector"]').click()
      cy.get('[data-testid="device-mobile"]').click()
      cy.get('[data-testid="preview-container"]').should('have.css', 'width', '375px')
    })
  })
  ...
  describe('Portfolio Editor', () => {
    ...
    it('should edit portfolio content', () => {
      ...
      cy.get('[data-testid="hero-subtitle"]').clear().type('Updated Hero Subtitle')
      ...
      cy.get('[data-testid="hero-title"]').should('have.value', 'Updated Hero Title')
    })
    ...
    it('should add and remove sections', () => {
      ...
      // Remove section
      cy.get('[data-testid="section-testimonials"]').find('[data-testid="remove-section"]').click()
    ...
    it('should change portfolio theme', () => {
      ...
      cy.get('[data-testid="preview-container"]').should('have.class', 'dark-theme')
      ...
      cy.get('[data-testid="preview-container"]').should('have.css', '--primary-color', 'rgb(59, 130, 246)')
    })
  ...
  describe('Portfolio Analytics', () => {
    it('should view portfolio analytics', () => {
      ...
      cy.get('[data-testid="unique-visitors"]').should('contain', 'Unique Visitors')
    ...
    it('should filter analytics by date range', () => {
      ...
      cy.get('[data-testid="analytics-data"]', { timeout: 10000 }).should('be.visible')
    })
  })
  ...
  describe('Error Handling', () => {
    it('should handle network errors gracefully', () => {
      ...
      cy.intercept('POST', '/api/portfolios', { statusCode: 500 }).as('createPortfolioError')
      ...
      cy.get('[data-testid="error-message"]').should('contain', 'Failed to create portfolio')
    })
    ...
    it('should handle validation errors', () => {
      ...
      // Verify validation errors
      cy.get('[data-testid="title-error"]').should('be.visible')
      cy.get('[data-testid="title-error"]').should('contain', 'Title must be at least 2 characters')
    })
  })
  ...
  describe('Accessibility Tests', () => {
    ...
    it('should have proper ARIA labels', () => {
      ...
      cy.get('[data-testid="device-selector"]').should('have.attr', 'aria-label')
      cy.get('[data-testid="zoom-controls"]').should('have.attr', 'aria-label')
      cy.get('[data-testid="save-portfolio"]').should('have.attr', 'aria-label')
    })
